...
const getMissingColumnFromError = (msg: string): string | null => {
  const p1 = msg.match(/column\s+"?([a-zA-Z0-9_]+)"?\s+of relation\s+"employees"\s+does not exist/i);
  if (p1) return p1[1];
  const p2 = msg.match(/could not find the '([a-zA-Z0-9_]+)'\s+column of 'employees'\s+in the schema cache/i);
  if (p2) return p2[1];
  const p3 = msg.match(/column\s+([a-zA-Z0-9_]+)\s+does not exist/i);
  if (p3) return p3[1];
  return null;
};
...
const sanitizeEmployeeInsertData = (data: EmployeeInsert): EmployeeInsert => ({
  ...
  date_of_birth: (data as { date_of_birth?: string | null }).date_of_birth === ""
    ? null
...
const Index = () => {
  ...
  const addEmployee = useMutation<EmployeeRow, Error, EmployeeInsert>({
    ...
    onError: (error) => {
      toast({
        title: "Error",
        description: error?.message ? `Failed to add employee: ${error.message}` : "Failed to add employee. Please try again.",
        variant: "destructive",
      });
